...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if new_status not in valid_statuses:
        return {
            'statusCode': 400,
            'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
            'body': json.dumps({'error': f'Недопустимый статус. Допустимые: {", ".join(valid_statuses)}'}),
            'isBase64Encoded': False
        }
    ...
    try:
        ...
        cur.execute(
            "UPDATE t_p43807817_real_estate_portal_3.applications SET status = %s WHERE id = %s RETURNING id",
